...
def get_value_at_x_y_day(data, x, y, date):
    ...
    """
    value_at_x_y_day = data.sel(x=x, y=y, time=date).srad.values
                        ...
                        for x, x_index in zip(dl_data.x, list(range(0, len(dl_data.x)))):
                            ...
                            for y, y_index in zip(dl_data.y, list(range(0, len(dl_data.y)))):
                                ...
                                if value_at_x_y_day != -1.0:
                                    rows[day_index]['srad'].append(
                                        {'x': x.item(), 'y': y.item(), 'srad': value_at_x_y_day}
                                    )
                                    ...
                                    if counter == batch_size:
                                        ...
                                        x_y_dayl_values = rows[0]['srad']

                                        update_statement = ', '.join([
                                            f"STRUCT({row['x']} AS x, {row['y']} AS y, {round(row['dayl'], 2)} AS srad)"
                                            for row in
                                            x_y_dayl_values
                                        ])
                                        sql = f"""
